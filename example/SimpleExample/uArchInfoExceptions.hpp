...
/**
 * uArchInfo parse error: the given unit for the instruction was not found by the uArchInfo
 * object.
 *
 * This can happen from a mis-spelling of the unit in the JSON, or from a need to update the
 * uArchInfo's list of supported units
 */
...
public:
            ...
            uArchInfoBaseException()
    {
        ...
        ss << "Instruction '" << mnemonic << "': "
           << " unit '" << unit_name << "' "
           << "is not known (uArchInfo object). Could be a mis-spelling in the JSON file";
        why_ = ss.str();
    }
};

/**
 * uArchInfo parse error: the given issue target for the instruction was not found by the uArchInfo
 * object.
 *
 * This can happen from a mis-spelling of the issue target in the JSON, or from a need to update the
 * uArchInfo's list of supported units
 */
...
/**
 * uArchInfo parsing error: the given ROB group begin/end stanza in the JSON is malformed
 *
 * This can happen from a mis-spelling of the rob_group key in the JSON. The form should be one
 * of the following:
 *
 * "rob_group" : ["begin"]
 * "rob_group" : ["end"]
