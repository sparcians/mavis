...
namespace mavis::utils
{
    ...
    struct smart_ptr_traits<std::shared_ptr<T>>
    {
        ...
        static ptr_type construct(Args... args) { return std::make_shared<U>(std::forward<Args>(args)...); }
    };

    template<typename T, typename Deleter>
    struct smart_ptr_traits<std::unique_ptr<T, Deleter>>
    {
        using ptr_type = std::unique_ptr<T, Deleter>;
        using element_type = typename ptr_type::element_type;

        template<typename U, typename ... Args>
        static ptr_type construct(Args... args) { return std::make_unique<U>(std::forward<Args>(args)...); }
    };
