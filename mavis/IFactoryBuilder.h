...
class IFactoryBuilder : public FactoryBuilderBase<IFactory<InstType,AnnotationType>, InstType, AnnotationType, AnnotationTypeAllocator>
{
public:
    typedef std::shared_ptr<IFactoryBuilder<InstType,AnnotationType,AnnotationTypeAllocator>>        PtrType;
...
public:
    explicit IFactoryBuilder(const FileNameListType& anno_files,
        ...
        FactoryBuilderBase<FactoryType,InstType,AnnotationType,AnnotationTypeAllocator>(anno_files, annotation_allocator, uid_list, anno_overrides)
    {
        ...
        // TODO: Clean this up. Should we move the UID registration inside the custom IFactory constructor?
        // I didn't do this here due to the need to make InstructionRegistry public and to forward-declare the
        // registry inside of the custom IFactory constructor
        //
        // Allow annotations for custom factories to be optional (suppress the exception on anno_registry_.findAnnotation())
        registry_["nop"].reset(
            new IFactory_NOP<InstType, AnnotationType>(einfo_nop, this->findAnnotation("nop", true)));
        ...
        //
        // Allow annotations for custom factories to be optional (suppress the exception on anno_registry_.findAnnotation())
        registry_["cmov"].reset(
            new IFactory_CMOV<InstType, AnnotationType>(einfo_cmov, this->findAnnotation("cmov", true)));
    ...
    typename IFactoryIF<InstType, AnnotationType>::PtrType build(const std::string& mnemonic,
                                                                 const std::string& factory_name,
                                                                 const std::string& xpand_name,
                                                                 const uint64_t stencil,
                                                                 ...
                                                                 ArgTypes&& ... args)
    {
        ...
        if (xpand_name.empty()) {
            ...
            // typename AnnotationType::PtrType panno = anno_registry_.findAnnotation(factory_name);
            // We suppress exceptions if we intend to try lookup by the mnemonic on failure
            typename AnnotationType::PtrType panno = this->findAnnotation(factory_name, mnemonic != factory_name);
            ...
            // ifact = FactoryBuilderBase<FactoryType, InstType, AnnotationType, AnnotationTypeAllocator>::findIFact(factory_name);
            ifact = this->findIFact(factory_name);
            if (ifact == nullptr) {
                // We need to build a new factory...
                ifact.reset(new FactoryType(factory_name, stencil, meta, std::forward(args)...));
            ...
            // This factory may cover several variants (e.g. csr's which use the JSON "factory" clause).
            // Add this instruction's annotation to the factory
            ...
            if (mnemonic != factory_name) {
                ...
                std::cerr << "*** '" << mnemonic << "': "
                          ...
                          << " overrides named factory '" << factory_name << "'"
                          << std::endl;
