...
namespace mavis
{
    ...
    template <class InstType> struct SharedPtrAllocator
    {
...
class Mavis
{
  public:
    ...
    using DecodeInfoType =
        typename mavis::IFactoryIF<InstType, AnnotationType>::IFactoryInfo::PtrType;
    ...
    using ContextRegistryType =
        mavis::ContextRegistry<InstType, AnnotationType, AnnotationTypeAllocator>;
  ...
  public:
     ...
     * @param inst_allocator       Reference to a memory allocator to use when creating InstType
     * objects
     * @param annotation_allocator Reference to a memory allocator to use when creating
     ...
     *
     */
    Mavis(const FileNameListType & isa_files, const FileNameListType & anno_files,
          ...
          const AnnotationTypeAllocator & annotation_allocator =
              mavis::SharedPtrAllocator<AnnotationType>()) :
        inst_allocator_(inst_allocator),
        annotation_allocator_(annotation_allocator),
        context_(annotation_allocator)
    {
        static_assert(std::is_same<typename InstTypeAllocator::InstTypePtr,
                                   typename InstType::PtrType>::value,
                      "You cannot use an InstTypeAllocator that returns a different pointer type "
                      "than the InstType thinks it is");
        static_assert(std::is_same<typename AnnotationTypeAllocator::InstTypePtr,
                                   typename AnnotationType::PtrType>::value,
                      "You cannot use an AnnotationTypeAllocator that returns a different pointer "
                      "type than the AnnotationTypeAllocator thinks it is");
        makeContext("BASE", isa_files, anno_files, uid_list, anno_overrides, inclusions,
                    exclusions);
        switchContext("BASE");
    }
    ...
    Mavis(const FileNameListType & isa_files, const FileNameListType & anno_files,
          ...
          const AnnotationTypeAllocator & annotation_allocator =
              mavis::SharedPtrAllocator<AnnotationType>()) :
        Mavis(isa_files, anno_files, uid_list, anno_overrides, {}, {}, inst_allocator,
    ...
    Mavis(const FileNameListType & isa_files, const FileNameListType & anno_files,
          ...
          const AnnotationTypeAllocator & annotation_allocator =
              mavis::SharedPtrAllocator<AnnotationType>()) :
        Mavis(isa_files, anno_files, {}, {}, inclusions, exclusions, inst_allocator,
              annotation_allocator)
    {
    }
    ...
    void makeContext(
        const std::string & name, const FileNameListType & isa_files,
    ...
    {
        context_.makeContext(name, isa_files, anno_files, uid_list, anno_overrides, inclusions,
                             exclusions);
    }
    ...
    bool hasContext(const std::string & name) { return context_.hasContext(name); }
    ...
    template <typename... ArgTypes>
    typename InstType::PtrType makeInstDirectly(const mavis::ExtractorDirectInfoIF & user_info,
                                                ArgTypes &&... args)
    {
        return dtrie_->makeInstDirectly(user_info, inst_allocator_,
     ...
     */
    template <typename... ArgTypes>
    typename InstType::PtrType makePseudoInst(const mavis::ExtractorDirectInfoIF & ex_info,
                                              ArgTypes &&... args)
    {
        ...
        {
            const typename mavis::IFactoryIF<InstType, AnnotationType>::IFactoryInfo::PtrType &
                info = ifact->getInfo(mnemonic, ex_info.clone());
    ...
    void morphInst(typename InstType::PtrType inst,
                   const mavis::ExtractorDirectInfoIF & user_info) const
    {
        dtrie_->morphInst(inst, user_info);
    }

    // Not const because getInfo will cache instruction information
    DecodeInfoType getInfo(const mavis::Opcode icode) { return dtrie_->getInfo(icode); }
    ...
    void setPseudoInstDisassembler(const mavis::InstructionUniqueID uid,
                                   const mavis::DisassemblerIF::PtrType & dasm)
    {
  ...
  private:
    ...
    typename mavis::IFactoryBuilder<InstType, AnnotationType, AnnotationTypeAllocator>::PtrType
        builder_;
    typename mavis::PseudoBuilder<InstType, AnnotationType, AnnotationTypeAllocator>::PtrType
        pseudo_builder_;
    typename mavis::DTable<InstType, AnnotationType, AnnotationTypeAllocator>::PtrType dtrie_;

  private:
    void print(std::ostream & os) const { os << *dtrie_; }
