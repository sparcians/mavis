...
public:
  explicit UnknownRISCVExtensionException(const uint32_t xlen,
                                          const std::string &ext)
      : UnknownExtensionExceptionBase("Unknown extension for riscv xlen=" +
                                      std::to_string(xlen) + ": " + ext) {}
};

class InvalidBaseExtensionException : public ExtensionManagerException {
private:
  static inline std::string genBaseExtensionString_(
      const std::unordered_set<std::string> &base_extensions) {
...
public:
  explicit InvalidBaseExtensionException(
      ...
      : ExtensionManagerException(
            ext + " is not a valid base extension. Valid extensions are: " +
...
public:
  explicit ISANotFoundInELFException(const std::string &elf)
      : ExtensionManagerException("Could not find a RISCV ISA string in " +
                                  elf) {}
};
...
private:
  ...
  void
  throwUnknownExtensionException_(const std::string &extension) const override {
    throw UnknownRISCVExtensionException(getXLEN(), extension);
  }

public:
  RISCVXLENState(const uint32_t xlen,
  ...
  void setExtensionVersion(const char ext, const uint32_t major_ver,
                           const uint32_t minor_ver) {
...
class RISCVExtensionManager
    : public ExtensionManager<RISCVExtensionInfo, RISCVXLENState> {
private:
  ...
  static bool getCharIfValid_(const std::string_view &isa_view,
                              char &front_char) {
    if (isa_view.empty()) {
      front_char = '\0';
      return false;
  ...
  static bool inSingleCharExtRange_(const std::string_view &isa_view,
                                    char &front_char) {
    return getCharIfValid_(isa_view, front_char) &&
           !isOneOf_(front_char, 'z', 's', 'x');
  }
  ...
  static uint32_t extractNumber_(std::string_view &isa_view) {
    ...
    const uint32_t val =
        std::stoul(std::string(isa_view.substr(0, number_end)));
    isa_view.remove_prefix(
        number_end == std::string_view::npos ? isa_view.size() : number_end);
    return val;
  }

  static std::pair<uint32_t, uint32_t>
  extractVersions_(std::string_view &isa_view) {
    ...
    if (!isa_view.empty() && isa_view[0] == 'p') {
      ...
      if (char next_char;
          !getCharIfValid_(isa_view, next_char) || !isdigit(next_char)) {
  ...
  static bool getBoolJSONValue_(const nlohmann::json &jobj,
  ...
  // it to an int
  uint32_t
  convertMultiArchString_(const std::string &multiarch_str) const override {
    return std::stoul(multiarch_str);
  }

  void processArchSpecificExtensionInfo_(
      RISCVXLENState &arch_extensions, const std::string &ext,
  ...
  static uint32_t readU32_(const char *ptr,
                           const ELFIO::endianness_convertor &convertor) {
    uint32_t val;
  ...
  static const char *findISAStringInELF_(const ELFIO::elfio &elf_reader) {
    ...
    static constexpr std::string_view riscv_attributes_sec_name(
        ".riscv.attributes");
    ...
    const ELFIO::section *riscv_attr_sec =
        elf_reader.sections[riscv_attributes_sec_name];
      ...
      if (getCharIfValid_(isa_view, ext) && isdigit(ext)) {
        try {
          ...
          throw InvalidISAStringException(
              isa_,
              "Invalid version number specified for extension " + base_isa);
        }
      }
      ...
      while (inSingleCharExtRange_(isa_view, ext)) {
        ...
        if (inSingleCharExtRange_(isa_view, ext) && isdigit(ext)) {
          try {
            ...
            throw InvalidISAStringException(
                isa_,
                "Invalid version number specified for extension " + ext_str);
          }
        }
      }
...
public:
  explicit RISCVExtensionManager(
      ...
      : ExtensionManager<RISCVExtensionInfo, RISCVXLENState>(
            unknown_extension_action) {}

  static RISCVExtensionManager
  fromISASpecJSON(const std::string &spec_json,
  ...
  static RISCVExtensionManager
  fromISA(const std::string &isa, const std::string &spec_json,
          const std::string &mavis_json_dir,
          const UnknownExtensionAction unknown_extension_action =
              UnknownExtensionAction::ERROR) {
  ...
  static RISCVExtensionManager
  fromELF(const std::string &elf, const std::string &spec_json,
          const std::string &mavis_json_dir,
          const UnknownExtensionAction unknown_extension_action =
              UnknownExtensionAction::ERROR) {
