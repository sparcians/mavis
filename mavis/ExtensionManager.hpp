...
class MetaExtensionUnexpectedJSONKeyException
    : public ExtensionManagerException {
public:
  explicit MetaExtensionUnexpectedJSONKeyException(const std::string &key)
      : ExtensionManagerException("Meta-extensions should not contain the " +
                                  key + " key") {}
};
...
public:
  ...
  InvalidISAStringException(const std::string &isa, const std::string &message)
      : ExtensionManagerException("Error processing isa string (" + isa +
                                  "): " + message) {}
};
...
public:
  explicit DuplicateExtensionException(const std::string &ext)
      : ExtensionManagerException("Extension " + ext +
...
public:
  MissingRequiredExtensionException(const std::string &ext,
...
public:
  ConflictingExtensionException(const std::string &ext,
                                const std::string &conflict_ext)
      : ExtensionManagerException(ext + " extension conflicts with " +
                                  conflict_ext) {}
};
...
private:
  ...
  void validateConstraints_(
      const std::vector<ExtensionInfoBasePtr> &constraint_extensions) const {
    ...
    if constexpr (assert_enabled) {
      it = std::find_if_not(
          constraint_extensions.begin(), constraint_extensions.end(),
          [](const ExtensionInfoBasePtr &ext) { return ext->isEnabled(); });
    } else {
      it = std::find_if(
          constraint_extensions.begin(), constraint_extensions.end(),
  ...
  void assertNotSelfReferential_(
      const std::vector<ExtensionInfoBasePtr> &exts) const {
    if (std::any_of(exts.begin(), exts.end(),
                    [this](const ExtensionInfoBasePtr &ext) {
                      return ext.get() == this;
                    })) {
      throw SelfReferentialException(extension_);
    }
...
public:
  ...
  addEnablingExtensions(
      const std::vector<std::shared_ptr<DerivedExtensionInfo>> &exts) {
    addEnablingExtensions(
        std::vector<ExtensionInfoBasePtr>(exts.begin(), exts.end()));
  }
...
enum class UnknownExtensionAction { ERROR, WARN, IGNORE };
...
using ExtensionMap =
    std::unordered_map<std::string, std::unique_ptr<ExtensionBase>>;
...
private:
  ...
  private:
    ...
    UnresolvedDependency(const DependencyType type,
        ...
        : type_(type), ext_(ext), dep_ext_(std::move(dep_ext)) {
  ...
  public:
    UnresolvedDependency(const DependencyType type,
        ...
        : UnresolvedDependency(
              type,
              std::unordered_set<std::string>(dep_ext.begin(), dep_ext.end()),
              ext) {}

    UnresolvedDependency(const DependencyType type, const std::string &dep_ext,
                         const ExtensionInfoPtr ext)
        : UnresolvedDependency(type, std::unordered_set<std::string>{dep_ext},
                               ext) {}
  ...
  virtual void
  throwUnknownExtensionException_(const std::string &extension) const {
    throw UnknownExtensionExceptionBase("Unknown extension specified: " +
                                        extension);
  }
...
protected:
  const ExtensionInfoPtr &
  getExtensionInfo_(const std::string &extension) const {
    try {
  ...
  bool recurseExtension_(const std::string &extension, Callback &&callback) {
    if (auto it = meta_extensions_.find(extension);
        it != meta_extensions_.end()) {
      for (const auto &child : it->second) {
        callback(child);
      }

      return true;
    } else if (auto alias_it = aliases_.find(extension);
...
public:
  ExtensionStateBase(const uint32_t arch_id,
      ...
      : arch_id_(arch_id), unknown_extension_action_(unknown_extension_action) {
  }
  ...
  ExtensionInfo &addExtension(const std::string &extension,
    ...
    const auto result = extensions_.emplace(
        extension, std::make_shared<ExtensionInfo>(extension, json));
  ...
  void addDependency(const std::string &extension, const std::string &dep) {
    ...
    if (const auto &dep_ext = getDependencyOrDefer_(type, dep, ext_ptr);
        dep_ext) {
      ...
      do {
        ...
        for (const auto &dep : dep_info.getDependentExtensions()) {
          if (auto meta_it = meta_extensions_.find(dep);
            ...
            retry = true;
            break;
          } else if (auto alias_it = aliases_.find(dep);
                     alias_it != aliases_.end()) {
            dep_info.removeDependentExtension(dep);
  ...
  void allowExtension(const std::string &extension) {
    recurseExtension_(
        extension, [this](const std::string &child) { allowExtension(child); });
  ...
  void blockExtension(const std::string &extension) {
    recurseExtension_(
        extension, [this](const std::string &child) { blockExtension(child); });
  ...
  void enableExtension(const std::string &ext) {
    if (!extensionAllowed_(ext)) {
      throw ExtensionNotAllowedException(ext, extension_allowlist_,
                                         extension_blocklist_);
    }

    bool is_meta = true;
    if (recurseExtension_(ext, [this](const std::string &child) {
...
protected:
  ...
  enum class ExtensionType { META, CONFIG, NORMAL };

  virtual void processArchSpecificExtensionInfo_(ExtensionState &,
                                                 const std::string &,
                                                 const nlohmann::json &,
                                                 const ExtensionType) const {}

  virtual uint32_t
  convertMultiArchString_(const std::string &multiarch_str) const = 0;

  virtual std::vector<uint32_t> convertMultiArchVector_(
      const std::vector<std::string> &multiarch_str_vec) const {
    std::vector<uint32_t> arches;
    arches.resize(multiarch_str_vec.size());

    std::transform(multiarch_str_vec.begin(), multiarch_str_vec.end(),
                   arches.begin(), [this](const std::string &multiarch_str) {
  ...
  static ManagerType
  fromELF_(const std::string &elf, const std::string &spec_json,
           const std::string &mavis_json_dir,
           const UnknownExtensionAction unknown_extension_action =
               UnknownExtensionAction::ERROR) {
    auto man = fromISASpecJSON_<ManagerType>(spec_json, mavis_json_dir,
  ...
  template <typename ManagerType>
  static ManagerType
  fromISA_(const std::string &isa, const std::string &spec_json,
           const std::string &mavis_json_dir,
           const UnknownExtensionAction unknown_extension_action =
               UnknownExtensionAction::ERROR) {
    auto man = fromISASpecJSON_<ManagerType>(spec_json, mavis_json_dir,
  ...
  template <typename ManagerType>
  static ManagerType
  fromISASpecJSON_(const std::string &spec_json,
                   const std::string &mavis_json_dir,
                   const UnknownExtensionAction unknown_extension_action =
                       UnknownExtensionAction::ERROR) {
...
private:
  ...
  getDependencyValue_(const nlohmann::json &obj) {
    return JSONVectorConverter<
        typename DependencyTraits<dep_type>::value_type>::get(obj);
  }

  template <bool is_normal_extension, DependencyType dep_type>
  static void processOptionalDependency_(ExtensionState &extensions,
    ...
    if (const auto it = obj.find(key); it != obj.end()) {
      ...
      extensions.template addDependency<dep_type>(
          ext, getDependencyValue_<dep_type>(*it));
    }
  }

  template <typename T>
  static T getRequiredJSONValue_(const nlohmann::json &jobj,
  ...
  void processExtension_(const nlohmann::json &ext_obj) {
    static constexpr bool is_normal_extension =
        extension_type == ExtensionType::NORMAL;
    static constexpr bool is_config_extension =
        extension_type == ExtensionType::CONFIG;

    const std::string ext =
        getRequiredJSONValue_<std::string>(ext_obj, "extension");
    ...
    for (const auto arch : arches) {
      auto &arch_extensions =
          extensions_.try_emplace(arch, arch, unknown_extension_action_)
              .first->second;
      ...
      processArchSpecificExtensionInfo_(arch_extensions, ext, ext_obj,
                                        extension_type);
      ...
      processOptionalDependency_<is_normal_extension, DependencyType::REQUIRED>(
          arch_extensions, ext, ext_obj);
      processOptionalDependency_<is_normal_extension,
                                 DependencyType::CONFLICTING>(arch_extensions,
                                                              ext, ext_obj);
    }
  }
  ...
  template <typename InstType, typename AnnotationType,
            typename InstTypeAllocator, typename AnnotationTypeAllocator,
            typename... MavisArgs>
  Mavis<InstType, AnnotationType, InstTypeAllocator, AnnotationTypeAllocator>
  constructMavis_(MavisArgs &&...mavis_args) const {
    return Mavis<InstType, AnnotationType, InstTypeAllocator,
...
public:
  ...
  void setISASpecJSON(const std::string &jfile,
    ...
    try {
      ...
      if (auto extensions_it = jobj.find("extensions");
          extensions_it != jobj.end()) {
  ...
  // NOTE: Allowlist changes will only apply to future invocations of setISA.
  // Existing enabled extensions are not affected.
  void allowExtensions(const uint32_t arch_key,
  ...
  // NOTE: Blocklist changes will only apply to future invocations of setISA.
  // Existing enabled extensions are not affected.
  void blockExtensions(const uint32_t arch_key,
                       const std::vector<std::string> &extensions) {
    for (const auto &ext : extensions) {
  ...
  constructMavis(
    ...
    return constructMavis_<InstType, AnnotationType, InstTypeAllocator,
                           AnnotationTypeAllocator>(
        anno_files, uid_list, anno_overrides, inclusions, exclusions,
  ...
  constructMavis(
    ...
    return constructMavis_<InstType, AnnotationType, InstTypeAllocator,
                           AnnotationTypeAllocator>(
        anno_files, uid_list, anno_overrides, inst_allocator,
  ...
  Mavis<InstType, AnnotationType, InstTypeAllocator, AnnotationTypeAllocator>
  constructMavis(
      const FileNameListType &anno_files, const MatchSet<Pattern> &inclusions,
      const MatchSet<Pattern> &exclusions,
      const InstTypeAllocator &inst_allocator = SharedPtrAllocator<InstType>(),
      const AnnotationTypeAllocator &annotation_allocator =
          SharedPtrAllocator<AnnotationType>()) const {
    return constructMavis_<InstType, AnnotationType, InstTypeAllocator,
                           AnnotationTypeAllocator>(anno_files, inclusions,
                                                    exclusions, inst_allocator,
                                                    annotation_allocator);
  }
