...
namespace mavis
{
    ...
    {
      public:
        IFactory_NOP(const InstMetaData::PtrType & einfo,
                     const typename AnnotationType::PtrType pui = nullptr) :
            IFactory<InstType, AnnotationType>("nop", 0, einfo, pui)
        {
            // NOTE: if the annotation provided is NULL, we will register a nullptr in the parent
            // IFactory's annotation map for this instruction. That's ok. This allows
            // IFactory::findAnnotation_() to take the fast path through its code
            einfo->setInstType(InstMetaData::InstructionTypes::INT,
                               InstMetaData::InstructionTypes::ARITH);
        }
    ...
    {
      public:
        IFactory_CMOV(const InstMetaData::PtrType & einfo,
                      const typename AnnotationType::PtrType pui = nullptr) :
            IFactory<InstType, AnnotationType>("cmov", 0, einfo, pui)
        {
            ...
            // IFactory's annotation map for this instruction. That's ok. This allows
            // IFactory::findAnnotation_() to take the fast path through its code
            einfo->setInstType(InstMetaData::InstructionTypes::CONDITIONAL,
                               InstMetaData::InstructionTypes::MOVE);
        }
