...
namespace mavis
{
    ...
    {
      public:
        explicit BadISAFile(const std::string & fname) : BaseException()
        {
            std::stringstream ss;
    ...
    {
      public:
        explicit BadAnnotationFile(const std::string & fname) : BaseException()
        {
            std::stringstream ss;
    ...
    {
      public:
        explicit BuildErrorMissingMnemonic(const std::string & filename) : BaseException()
        {
            std::stringstream ss;
            ...
            BaseException()
        {
            ...
            ss << "The entry with stencil '" << stencil_str
               << "'"
                  " is missing a mnemonic in JSON ISA file '"
               << filename << "'";
            why_ = ss.str();
        }
    };
    ...
    {
      public:
        BuildErrorUnknownForm(const std::string & filename, const std::string & mnemonic,
        ...
        explicit BuildErrorUnknownForm(const std::string & form_name) : BaseException()
        {
            std::stringstream ss;
     ...
     *
     * This can usually be resolved by either making one of the conflicting instructions more
     * specific (e.g. adding missing fixed field definitions in the JSON), or introducing new
     * decode-only forms
     */
    class BuildErrorInstructionAlias : public BaseException
    {
      public:
        BuildErrorInstructionAlias(uint64_t istencil, const std::string & mnemonic,
                                   const std::string & node_name) :
            BaseException()
        {
            std::stringstream ss;
            ss << "ALREADY A LEAF for stencil: 0x" << std::hex << istencil << ", '" << mnemonic
               << "'"
    ...
    {
      public:
        BuildErrorOpcodeConflict(const std::string & mnemonic, uint64_t istencil) : BaseException()
        {
            std::stringstream ss;
            ss << "Instruction '" << mnemonic << "': "
               << "opcode 0x" << std::hex << istencil
               << " is the same as another instruction, with incompatible form";
            why_ = ss.str();
        }
    };

    /**
     * IFactory builder error: expansion named in JSON for an instruction has not yet been
     * processed.
     *
      ...
      public:
            ...
            BaseException()
        {
            ...
            ss << "Instruction '" << mnemonic << "': "
               ...
               << "not yet declared. Move this instruction after the named factory in the JSON "
                  "file";
            why_ = ss.str();
        }
    };
    ...
    /**
     * InstMetaData build error: the given type for the instruction was not found by the
     * InstMetaData object.
     *
    ...
    {
      public:
        BuildErrorMalformedISAExtension(const std::string & mnemonic,
            ...
            BaseException()
        {
            ...
            ss << "Instruction '" << mnemonic << "': "
               ...
               << "is malformed. Needs to be an extension letter optionally preceded by a power-2 "
                  "width";
            why_ = ss.str();
        }
    };
    ...
    /**
     * InstMetaData build error: the given ISA extension string does not provide a valid extension
     * letter
     *
     * This can happen from a type-o in the ISA field of the JSON, or from a need to update the
     * InstMetaData's list of supported ISA extension letters
    ...
    {
      public:
        BuildErrorInvalidISAExtension(const std::string & mnemonic, const std::string & isa_string,
    ...
    {
      public:
        BuildErrorInvalidISAWidth(const std::string & mnemonic, const std::string & isa_string,
    ...
    {
      public:
        BuildErrorInvalidDataSize(const std::string & mnemonic, int32_t data) : BaseException()
        {
            std::stringstream ss;
            ss << "Instruction '" << mnemonic << "': "
               << " data size '" << data << "' "
    ...
    {
      public:
        explicit BuildErrorDuplicateMnemonic(const std::string & mnemonic) : BaseException()
        {
            std::stringstream ss;
            ss << "Instruction '" << mnemonic << "': "
    ...
    {
      public:
        BuildErrorDuplicateAnnotation(const std::string & mnemonic,
    ...
    class BuildErrorMissingAnnotation : public BaseException
    {
      public:
        ...
        BuildErrorMissingAnnotation(const std::string & mnemonic) : BaseException()
        {
            std::stringstream ss;
            ss << "Instruction or factory named '" << mnemonic << "'"
               << " has no matching annotation. Check the ISA and annotation JSON files for "
                  "correctness";
            why_ = ss.str();
        }
    };
    ...
    {
      public:
        BuildErrorOverlayBaseNotFound(const std::string & olay_mnemonic,
                                      const std::string & base_mnemonic,
    ...
    /**
     * \brief Overlay annotation build error: This may not really be an error, if the user has not
     * provided a matching annotation for this instruction. We raise the exception, and allow the
     * user to decide whether to ignore it
     */
    class BuildErrorOverlayMissingAnnotation : public BaseException
    {
      public:
        BuildErrorOverlayMissingAnnotation(const std::string & olay_mnemonic,
                                           const std::string & base_mnemonic) :
            BaseException()
        {
            std::stringstream ss;
    ...
    {
      public:
        explicit BuildErrorOverlayMissingBase(const std::string & olay_mnemonic) : BaseException()
        {
            std::stringstream ss;
            ss << "Overlay instruction '" << olay_mnemonic << "': "
    ...
    {
      public:
        explicit BuildErrorOverlayMissingMatch(const std::string & olay_mnemonic) : BaseException()
        {
            std::stringstream ss;
            ss << "Overlay instruction '" << olay_mnemonic << "': "
    ...
    {
      public:
        explicit BuildErrorOverlayBadMatchSpec(const std::string & olay_mnemonic) : BaseException()
        {
            std::stringstream ss;
            ss << "Overlay instruction '" << olay_mnemonic << "': "
    ...
    {
      public:
        InvalidExtractorSpecialFieldID(const std::string & fname, Opcode icode) : BaseException()
        {
            std::stringstream ss;
        ...
        InvalidExtractorSpecialFieldID(const std::string & mnemonic) : BaseException()
        {
            std::stringstream ss;
            ss << "Special fields are not valid for instruction '" << mnemonic << "'."
               << " This can happen if you're trying to get a special field from a direct "
                  "extraction object";
            why_ = ss.str();
        }
    };
    ...
    class UnsupportedExtractorSpecialFieldID : public BaseException
    {
      public:
        ...
        UnsupportedExtractorSpecialFieldID(const std::string & mnemonic,
                                           const UnsupportedExtractorSpecialFieldID & prior) :
            BaseException(),
            sfid_name_(prior.sfid_name_),
            opcode_(prior.opcode_)
        {
            std::stringstream ss;
    ...
    {
      public:
        InvalidRegisterNumber(const std::string & mnemonic, uint32_t regnum) : BaseException()
        {
            std::stringstream ss;
    ...
    {
      public:
        BuilderInstIDListAlreadyRegistered(const std::string & mnemonic) : BaseException()
        {
            std::stringstream ss;
            ss << "Instruction '" << mnemonic << "'"
    ...
    {
      public:
        explicit UnknownMnemonic(const std::string & mnemonic) : BaseException()
        {
            std::stringstream ss;
    ...
    /**
     * Invalid pseudo instruction mnemonic for ExtractorDirectInfo (the user-supplied mnemonic does
     * not match any factory known to the PseudoBuilder)
     */
    class UnknownPseudoMnemonic : public BaseException
    {
      public:
        explicit UnknownPseudoMnemonic(const std::string & mnemonic) : BaseException()
        {
            std::stringstream ss;
    ...
    {
      public:
        explicit UnknownPseudoUID(const InstructionUniqueID uid) : BaseException()
        {
            std::stringstream ss;
     ...
     *
     * We can get into this situation for complex fixed/ignore combinations in the instruction's
     * JSON ISA spec. I've seen it for mv (a special case of addi with an immediate of 0). The
     * problem arose because the form used for decode (not the extraction xform) had two fields to
     * cover the immediate -- and if either of these fields are non-zero, the instruction needed to
     * decode to addi. Only when BOTH fields are 0 do we decode as a mv. The "either-or" concept
     * wasn't representable in Mavis.
     *
     * Short story, if you get here, you'll need to think about handling the instruction using an
     * overlay
     */
      ...
      public:
            ...
            BaseException()
        {
            ...
            ss << "Cannot decode opcode 0x" << std::hex << opcode
               << " due to DTable lookup error (malformed TRIE). Default/closest mnemonic is '"
               << closest_mnemonic << "'";
            why_ = ss.str();
        }
    };
    ...
    {
      public:
        explicit UnknownOpcode(uint64_t opcode) : BaseException()
        {
            std::stringstream ss;
            ss << "Cannot decode opcode 0x" << std::hex << opcode;
            why_ = ss.str();
     ...
     * (This is mainly for compressed instructions which have reserved immediate or register field
     * values)
     */
    class IllegalOpcode : public BaseException
    {
      public:
        explicit IllegalOpcode(const std::string & mnemonic, uint64_t opcode) : BaseException()
        {
            std::stringstream ss;
            ss << "Opcode 0x" << std::hex << opcode << " is illegal for instruction '" << mnemonic
               << "'";
            why_ = ss.str();
        }
    };
    ...
    {
      public:
        explicit ContextAlreadyExists(const std::string & name) : BaseException()
        {
            std::stringstream ss;
    ...
    {
      public:
        explicit UnknownContext(const std::string & name) : BaseException()
        {
            std::stringstream ss;
            ss << "Mavis context '" << name << "'"
               << " is not known";
    ...
    {
      public:
        explicit OperandInfoInvalidFieldID(const std::string & fid_name) : BaseException()
        {
            std::stringstream ss;
