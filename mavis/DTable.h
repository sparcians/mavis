...
namespace mavis
{
    ...
    // TODO: Special extractors for SFENCE.VMA (TLB invalidates) and JALR (LR detect for RAS
    // push/pop) Special cases are determined by register values and register value relationships Do
    // we need an extractor facility for test condition lambda's?
     ...
     *
     * The DTable is constructed by reading in the ISA JSON files. Below is an example of an ISA
     * JSON entry for a instruction from the rv64a extension:
     *
     *     "mnemonic" : "amoxor.d",
     *     "tags" : ["a", "g"],
     *     "form" : "AMO",
     ...
     *
     * The Stencil provides the fixed values of the opcode fields. For an instruction to be decoded
     * as an amoxor.d instruction, its opcode field values must match the values in the stencil
     * exaclty.
     *
     ...
     *
     * When constructing the DTable, a new node is created for every unique opcode field value. For
     * the rv64a extension, this results in a 3-level table (one level for each opcode field). There
     * is one unique value for the opcode field (0x2F) and two unique values for the func3 field
     * (0x2 and 0x3). For each unique value of func3, there is a node for func5 with child nodes for
     * each unique vaue of func5. This results in 22 leaf nodes in the DTable, one for each
     * instruction in the rv64a extension.
     *
     ...
     * extension have the same opcode value, so the next step is to get the value of the func3
     * field. The value of func3 will determine which func5 node to go to. Since func5 is the last
     * opcode field, the next node will be a leaf node with the instruction Extractor.
     *
    ...
    class DTable
    {
      ...
      private:
        ...
        template <typename ObjectType, uint32_t Size, bool CollectStats = false> class Cache
        {
          private:
            struct Line
            {
      ...
      public:
        ...
        {
            const typename IFactoryIF<InstType, AnnotationType>::IFactoryInfo::PtrType & ohandle =
                ocache_->lookup(icode);
            if (ohandle == nullptr)
            {
                const typename IFactory<InstType, AnnotationType>::IFactoryInfo::PtrType &
                    new_ohandle = root_->getInfo(icode);
                                                     ...
                                                     ArgTypes &&... args)
        {
            ...
            if (std::string(inst->getMnemonic()) != tinfo.getMnemonic())
            {
                ...
                typename IFactoryIF<InstType, AnnotationType>::PtrType ifact =
                    builder_->build(tinfo.getMnemonic(), tinfo.getMnemonic(), "", 0, einfo);
        ...
        void morphInst(typename InstType::PtrType inst, const ExtractorDirectInfoIF & ex_info) const
        {
            ...
            typename IFactory<InstType, AnnotationType>::PtrType ifact =
                builder_->findIFact(ex_info.getMnemonic());
            if (ifact == nullptr)
            {
    ...
    operator<<(std::ostream & os,
               const DTable<InstType, AnnotationType, AnnotationTypeAllocator> & dt)
    {
        dt.print(os);
        return os;
    }
