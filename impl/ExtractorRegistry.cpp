...
namespace mavis {
    ...
    const ExtractorIF::PtrType &ExtractorRegistry::getExtractor(const std::string &fname)
    {
        static const std::map<std::string, ExtractorIF::PtrType> EXTRACTOR_REGISTRY = {
            ...
            {Form_C2_sp_load_word_pair::name,     std::make_shared<Extractor<Form_C2_sp_load_word_pair>>()},
            ...
            {Form_C2_sp_store_word_pair::name,    std::make_shared<Extractor<Form_C2_sp_store_word_pair>>()},
