...
namespace mavis
{
    ...
    /**
     * DTableBranchBuilder: this is used for doing special "manual" builds of branches. The class
     * wrapper to populate() allows us to partially specialize the template (e.g. specializing on
     * Form but not on InstType) (templated functions cannot be partially specialized in C++17)
     *
     * @tparam FormType
     * @param mnemonic
    ...
    class DTableBranchBuilder<InstType, AnnotationType, Form_R>
    {
      public:
        ...
        // operator()(const IFactoryIF<InstType, AnnotationType> *root, const Opcode istencil, const
        // FieldNameSetType &ignore_set);
        static inline typename IFactoryIF<InstType, AnnotationType>::PtrType
        populate(typename IFactoryIF<InstType, AnnotationType>::PtrType root, Opcode istencil,
    ...
    void DTable<InstType, AnnotationType, AnnotationTypeAllocator>::buildSpecial_(
        ...
        const std::string & xpand_name, const ExtractorIF::PtrType & override_extractor,
        const InstMetaData::PtrType & einfo)
    {
        // IFactoryIF<InstType, AnnotationType> *currNode = DTableBranchBuilder<InstType,
        // FormType>()(root_, istencil, ignore_set);
        IFactoryIF<InstType, AnnotationType>* currNode =
            DTableBranchBuilder<InstType, AnnotationType, FormType>::populate(root_, istencil,
                                                                              ignore_set);
        // buildLeaf_<FormType>(currNode, mnemonic, istencil, flist, factory_name,
        // override_extractor);
        const FormBase* form_wrap = forms_.getFormWrapper(FormType::name);
        buildLeaf_(form_wrap, currNode, mnemonic, istencil, flist, factory_name, xpand_name,
                   override_extractor, einfo);
    }
    ...
    DTableBranchBuilder<InstType, AnnotationType, Form_R>::populate(
        typename IFactoryIF<InstType, AnnotationType>::PtrType root, const Opcode istencil,
        const FieldNameSetType & ignore_set)
    {
        ...
        if (currNode->getNode(istencil) == nullptr)
        {
            currNode->addIFactory(istencil,
                                  typename IFactoryIF<InstType, AnnotationType>::PtrType(
                                      new IFactoryDenseComposite<InstType, AnnotationType>(
