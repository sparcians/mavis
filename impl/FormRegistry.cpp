...
namespace mavis {
    ...
    static const std::map<std::string, const FormWrapperIF*> form_registry_ = {
        ...
        {Form_V_uimm6::getName(),    new FormWrapper<Form_V_uimm6>()}
    };

    const FormWrapperIF* FormRegistry::findFormWrapper(const std::string& fname)
    {
    ...
    const FormWrapperIF* FormRegistry::getFormWrapper(const std::string& fname)
    {
        const auto itr = form_registry_.find(fname);
        if (itr == form_registry_.end()) {
