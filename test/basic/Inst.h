...
template <typename AnnotationType> class Instruction
{
  ...
  public:
    ...
    uint64_t getImmediate() const { return dinfo_->getImmediate(); }

    mavis::MatchSet<mavis::Tag> getTags() const { return dinfo_->getTags(); }

    uint32_t getDataSize() const { return dinfo_->getDataSize(); }

    auto getSourceOpInfoList() const { return dinfo_->getSourceOpInfoList(); }

    auto getDestOpInfoList() const { return dinfo_->getDestOpInfoList(); }
