...
public:
    ...
    void morph(const typename mavis::OpcodeInfo::PtrType& new_dinfo, const typename AnnotationType::PtrType& new_ui) {
        // dinfo_.reset(new_dinfo);
        dinfo_ = new_dinfo;
        //uinfo_.reset(new_ui);
        uinfo_ = new_ui;

        // TBD: Instruction user may need to reset any information cached with this instruction
    }

    /**
     * User code to "recycle" an instruction (which DTable has cached and is attempting to reuse)
     */
    void recycle() {}

    typename mavis::OpcodeInfo::PtrType getOpInfo() const {
        return dinfo_;
    }

    std::string getMnemonic() const {
        return dinfo_->getMnemonic();
    }

    std::string dasmString() const {
        return dinfo_->dasmString();
    }
...
private:
    ...
    void print(std::ostream& os) const {
        ...
        os << dinfo_->getMnemonic()
           << ", src[" << dinfo_->numSourceRegs() << "]: " << dinfo_->getSourceRegs()
           << " (addr: " << dinfo_->getSourceAddressRegs() << ")"
           << ", dst: " << dinfo_->getDestRegs()
           << ", data size: " << std::dec << dinfo_->getDataSize();
        if (uinfo_ != nullptr) {
            os << ", uInfo: " << *uinfo_;
        }
        os.flags(os_state);
